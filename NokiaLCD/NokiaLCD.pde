...
//#include <OneWire.h>  // One Wire library for iButton clock
#include "time.h"
...
// Not using bitmap image at this time

// #define _SHOW_RGB_IMAGE_
// void	DisplayRGBimage();

// Not using push buttons at this time
#define	kSwitch1_PIN	3
#define	kSwitch2_PIN	4
#define	kSwitch3_PIN	5

//OneWire  one_wire_bus(2);    // Initialize OneWire bus pin 2

const float pi = 3.141592653;

// Store unique 1Wire sensor serial code per device type
//byte onewire_clock_device[2];                  // DS1904 iButton real time clock

// Clock parameters
timedate td;                                   // Time - date structure from Maxim SDK
analoghand second;                             // Analog seconds hand structure
analoghand minute;                             // Analog minutes hand structure
analoghand hour;                               // Analog hour hand structure 

char date_lcd_text[9] = {""};                  // Date text ASCII for LCD
char time_lcd_text[9] = {""};                  // Time text ASCII for LCD  
byte daylight_savings_flag = 1;                // 0 = normal time, 1 = daylight_savings_flag
byte real_time_clock_poll_flag = 0;            // Track to poll DS1994 one time and then use Arduino for clock to address bus polling issues
byte datalog_clock_status_flag = 0;            // Datalogger clock on or off status 
unsigned long arduino_time_sync = 0;           // Store arduino millisecond count from startup until when DS1994 clock is started to sync time 
unsigned long total_seconds = 0;               // Total Seconds from 1970, used with DS1994 clock
unsigned long real_time_clock= 0;              // Get time (total seconds) from DS1994 iButtom to sync with Arduino
int time_drift_correct = 0;                    // Variable to correct drift in real time DS1994 iButton clock in seconds
...
unsigned long	startTime;
	...
	LCD_Print_Hello_World();


        // Draw Analog Clock Face  
        
        // Frame
//        LCDSetCircle(clock_center_x, clock_center_y, clock_radius, WHITE);
//        LCDSetCircle(clock_center_x, clock_center_y, clock_radius - 1, WHITE);        
//        LCDSetCircle(clock_center_x, clock_center_y, clock_radius - 2, WHITE);       
...
//          LCDSetLine(tick_x_in, tick_y_in, tick_x_out, tick_y_out, WHITE);
//        }
//        // Face Characters
//        LCDPutStr("9 ", clock_center_x - 8, 106, LARGE, WHITE, BLACK);
//        LCDPutStr(" 3", clock_center_x - 8, 25, LARGE, WHITE, BLACK);
//        LCDPutStr("12", clock_center_x + 34, clock_center_y, LARGE, WHITE, BLACK);
//        LCDPutStr("6", clock_center_x - 50, clock_center_y - 4, LARGE, WHITE, BLACK); 
       
   

   // Initialize Minute hand struct
   // Analog Clock Structure

//   minute.base_radius = 3;
//   minute.hand_radius = 33;
//   minute.oldtime = 0;
//   second.hand_radius = 33;
//   hour.base_radius = 3;
//   hour.hand_radius = 25;   
...
void LCD_Print_Hello_World()
{
  ...
  LCDClear(WHITE);
  LCDPutStr("Hello, RiCH@rd?!", coord_x, coord_y, Font_Size, FG, BG);
}
...
//   td.second = 0;
//   td.year = 0;
//   int utc_cst = -18000; // Central time zone utc - 5 hours in seconds
//
//    // Get time from DS1904 one time to reduce one wire polling and have Arduino track time
//    if (real_time_clock_poll_flag == 0){ // Get time 
//
//       SearchOneWireDevices();
//       delay(1000);
//
//       // Account for Daylight Savings Time
//       if (daylight_savings_flag == 0){
//         utc_cst = utc_cst - 3600;  // Subtract one hour or 3600 seconds to account for no daylight savings
//         }
//      
//        // Get time from DS1904 onboard clock
//         real_time_clock = GetTimeRTC(onewire_clock_device)+utc_cst+time_drift_correct; // Get total seconds from 1970 reference with time zone correction
//Serial.println(real_time_clock);
//
//        // Check to see if seconds is a reasonable number 
//        if (real_time_clock < 1270641600) { 
//        //    real_time_clock = 1276344900; //12:15:00 6/12/10
//            real_time_clock = 1276422000; //9:40
//
////            real_time_clock = 1276419900; // 9:05 6/13/10
//        }
//        else if (real_time_clock > 1607752800) {
//        //    real_time_clock = 1276419900; // 9:05 6/13/10   
//            real_time_clock = 1276422000; //9:40
//         // real_time_clock = 1276344900; //12:15:00 6/12/10
//        }         
//
//        arduino_time_sync = millis(); // Get total runtime up to this point to sync Arduino clock with onboard DS1994 iButton clock
//        real_time_clock_poll_flag = 1;         // Clock set, do not reload time from DS1994 iButton   
//      }
//
//      // Calculate current run time since clock was synced with iButton
//      unsigned long ArduinoTime = (millis() - arduino_time_sync)*.001;
//
//      // Calculate total seconds from 1970 (time from iButton clock + Arduino run time since sync
//      total_seconds =  real_time_clock + ArduinoTime;
//
//      SecondsToDate(&td, total_seconds);  // Convert total seconds into a usable time and date
//
//
//
//   if (total_seconds != second.oldtime){
//
//      // Draw Seconds Hand
//      
...
//     
//      LCDSetLine(clock_center_x, clock_center_y, second.end_x, second.end_y, BLACK); // Draw new seconds hand    
//
//     // Draw New Seconds Hand
...
//      draw_hour(WHITE);
//      
//      second.oldtime = total_seconds;
//
//      // Display Digital Time
//      LCDPutStr(time_lcd_text, 5, 113, MEDIUM, WHITE, BLACK);
//      LCDPutStr(date_lcd_text, 5, 40, MEDIUM, WHITE, BLACK);
//
//   }
// 
//      // Draw Minutes Hand
//
//       minute.angle = (2 * pi / 60) * td.minute;
//
//       if (total_seconds >= (minute.oldtime + 60)){
//
//         // Erase old minutes Hand
//         draw_minute(BLACK);  
//         // Calculate Minute Hand Base
//         minute.base_x = minute.base_radius * cos (minute.angle - pi / 2) + clock_center_x;
//         minute.base_y = minute.base_radius * sin (minute.angle - pi / 2) + clock_center_y;
//         minute.base_x1 = minute.base_radius * cos (minute.angle + pi / 2) + clock_center_x;
//         minute.base_y1 = minute.base_radius * sin (minute.angle + pi / 2) + clock_center_y;
//         // Calculate Minute Hand Point
//         minute.end_x = minute.hand_radius * cos (minute.angle) + clock_center_x;
//         minute.end_y = minute.hand_radius * sin (minute.angle) + clock_center_y;
//         // Draw new minutes Hand
//         draw_minute(WHITE);                         
//         // Redraw hour hand
//         draw_hour(WHITE);
//
//         minute.oldtime = total_seconds;
//
//       }
//   
//       float minfloat = td.minute; // Convert minute short to float
//       
//       hour.angle = (2 * pi / 12) * (td.hour + (minfloat/60)); // Find angle for hour hand
//       
//       if (total_seconds >= (hour.oldtime + 60)){
//
//         // Erase old Hour Hand
//         draw_hour(BLACK);
//         // Calculate hour Hand Base
//         hour.base_x = hour.base_radius * cos (hour.angle - pi / 2) + clock_center_x;
//         hour.base_y = hour.base_radius * sin (hour.angle - pi / 2) + clock_center_y;
//         hour.base_x1 = hour.base_radius * cos (hour.angle + pi / 2) + clock_center_x;
//         hour.base_y1 = hour.base_radius * sin (hour.angle + pi / 2) + clock_center_y;
//         // Calculate hour Hand Point
//         hour.end_x = hour.hand_radius * cos (hour.angle) + clock_center_x;
//         hour.end_y = hour.hand_radius * sin (hour.angle) + clock_center_y;
//         // Draw new hours Hand
//         draw_hour(WHITE);
//         
//         hour.oldtime = total_seconds; 
//
//    }
//         
//       
...
//#endif
//
//// Not using shield Buttons at this time...
//
//	if (s1)
//	{
...
{
  LCDSetLine(minute.base_x, minute.base_y, minute.base_x1, minute.base_y1, color); 
  LCDSetLine(minute.base_x, minute.base_y, minute.end_x, minute.end_y, color); 
  LCDSetLine(minute.base_x1, minute.base_y1, minute.end_x, minute.end_y, color); 
}

// Draw Hour Hand
void draw_hour(int color)
{
   LCDSetLine(hour.base_x, hour.base_y, hour.base_x1, hour.base_y1, color); 
   LCDSetLine(hour.base_x, hour.base_y, hour.end_x, hour.end_y, color); 
   LCDSetLine(hour.base_x1, hour.base_y1, hour.end_x, hour.end_y, color); 
}
